...
static int retrieve_color(t_display *map_display, int x, int y, int diff)
{
	...
	{
		relative_y_in_texture = get_relative_y(map_display, x, y, diff);
...
static int get_relative_y(t_display *map_display, int x, int y, int diff)
{
	...
	relative_y_in_texture = (int)((float)((float)(y - diff) / ((float)map_display->ray_size[x])) * y_size);
	relative_y_in_texture %= y_size;
	return (relative_y_in_texture);
}
